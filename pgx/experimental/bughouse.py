...
def to_fen(state: State):
    ...
    def fn(board_num):
        pb = jnp.rot90(state._board[0][board_num].reshape(8, 8), k=1)
        pocket = state._pocket[0][board_num]

        if state._turn[0][board_num] == 1:
        ...
        fen += " "
        # turn
        fen += "w " if state._turn[0][board_num] == 0 else "b "
        # castling
        can_castle_queen_side = state._can_castle_queen_side[0][board_num]
        can_castle_king_side = state._can_castle_king_side[0][board_num]
        if state._turn[0][board_num] == 1:
        ...
        fen += " "
        # en passant
        en_passant = state._en_passant[0][board_num]
        if state._turn[0][board_num] == 1:
        ...
        if ep == -1:
            ...
            fen += str(ep % 8 + 1)
        fen += " "
        fen += str(state._halfmove_count[0][board_num].item())
        fen += " "
        fen += str(state._fullmove_count[0][board_num].item())
        return fen
